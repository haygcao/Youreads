...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    if (process.env.wdwhdhd && process.env.wdwhdhd.indexOf('\n') > -1) {
      wdwhdhdArr.push(process.env.wdwhdhd.split('\n'))
    } else {
      wdwhdhdArr.push(process.env.wdwhdhd.split())
    };
    if (process.env.wdwhdbody && process.env.wdwhdbody.indexOf('\n') > -1) {
      wdwhdbodyArr.push(process.env.wdwhdbody.split('\n'))
    }  else if(process.env.wdwhdbody !== undefined) {
      wdwhdbodyArr.push(process.env.wdwhdbody.split())
    };
    if (process.env.wdwhdspbody && process.env.wdwhdspbody.indexOf('\n') > -1) {
      wdwhdspbodyArr.push(process.env.wdwhdspbody.split('\n'))
    } else if(process.env.wdwhdspbody !== undefined) {
      wdwhdspbodyArr.push(process.env.wdwhdspbody.split())
    };
    ...
    console.log(`------------- 共${wdwhdhdArr.length}个账号-------------\n`)
      for (let i = 0; i < wdwhdhdArr.length; i++) {
        if (wdwhdhdArr[i]) {
          wdwhdhd = String(wdwhdhdArr[i]);
          wdwhdbody = String(wdwhdbodyArr[i]);
          wdwhdspbody = String(wdwhdspbodyArr[i]);
          $.index = i + 1;
          console.log(`\n开始【我的网红店${$.index}】`)
